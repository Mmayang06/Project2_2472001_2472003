                ...
                filteredInventoryData.forEach((room) => {
                    ...
                    if (currentItems.length === 0) {
                        ...
                        currentItems.forEach(item => {
                            ...
                            cardsHtml += `
                            <div onclick="openGroupModal(${room.id}, '${item.id_detail.replace(/'/g, "\\'")}')" class="cursor-pointer block bg-white/35 backdrop-blur-lg border border-white/40 rounded-xl overflow-hidden hover:-translate-y-1 hover:scale-[1.02] hover:border-[#6196aa] hover:shadow-xl transition-all duration-300 group relative">
                                ${badgeHtml}
